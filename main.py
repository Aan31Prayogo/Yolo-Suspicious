#import library dan file yang dibuthkan
#library untuk python
...
import json						#library json obhect
...
def set_relay_active():
	try:
		...
		GPIO.output(relay,0)  # kembalikan keadan relay
	except Exception as e:
...
def open_camera():
	...
	try:
		...
		cap.set(4,CAMERA_HEGIHT)				#set resolusi (height) kamera
		...
		while True:
			...
			fps = str(int(fps))
			gray_frame=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)   #convert ke graysclae
			...
			elif key==ord('c'):  # tekan c untuk exit
				capture_camera()
